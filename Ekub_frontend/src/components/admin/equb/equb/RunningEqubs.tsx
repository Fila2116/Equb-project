...
interface RunningEqubsProps {
  searchTerm: string;
  equbTypeFilter: "Daily" | "Weekly" | "Monthly" | "all";
  equbTypeCategory: "Finance" | "Special Finance" | "Finance and Other" | "all";
}

const RunningEqubs: React.FC<RunningEqubsProps> = ({
  searchTerm,
  equbTypeFilter,
  equbTypeCategory,
}) => {
  const dispatch = useAppDispatch();
  const { running, totalPages, isLoading, error } = useAppSelector(
    (state) => state.equbs
  );
  ...
  const filteredEqubs = running.filter((equb) => {
  ...
  return (
    <div className="min-w-fit my-2">
      ...
      />
      {editEqub && <EditEqubPopup equb={editEqub} setOpen={handleClosePopup} />}
    </div>
  );
};
