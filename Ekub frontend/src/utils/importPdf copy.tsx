...
const exportPDF = async ({
  ...
  doc.setFontSize(12);
  doc.text("Customer Receipt", marginLeft + (contentWidth / 2), headerPadding + 45, { align: "center" });
  doc.setFontSize(10);
...
let currentYRight=currentY;
 currentY += 15;
  ...
  if (cust.name != "") {
currentYRight += 15;
    doc.text(`Customer Name: ${cust.name || ""}`, marginLeft + columnWidth, currentYRight); currentYRight += 10;
    doc.text(`Phone Number: ${cust.phone || ""}`, marginLeft + columnWidth, currentYRight); currentYRight += 10;
    doc.text(`Start Date : ${cust.startDate || ""}`, marginLeft + columnWidth, currentYRight); currentYRight += 10;
    doc.text(`End Date : ${cust.endDate || ""}`, marginLeft + columnWidth, currentYRight); currentYRight += 10;
    doc.text(`Equb : ${cust.equbName || ""}`, marginLeft + columnWidth, currentYRight); currentYRight += 10;
  }
  if (cust.name == "") {
    if (cust.equbName) {

      doc.text(`Equb : ${cust.equbName || ""}`, marginLeft + columnWidth, currentYRight);
    }
    currentYRight += 15;
    if (cust.startDate) {
      doc.text(`Start Date : ${cust.startDate || ""}`, marginLeft + columnWidth, currentYRight); currentYRight += 15;
    }
    if (cust.startDate) {
      doc.text(`End Date : ${cust.endDate || ""}`, marginLeft + columnWidth, currentYRight); 
    }
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(15);
  doc.text("Payment / Transaction Information", pageWidth / 2, currentY, { align: "center" });
  currentY += 10;
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(12);
  doc.text(`Total Paid: ${Number(config.totalPaid).toFixed(2)}`, marginLeft, currentY, { align: "left" });

  currentY += 20;
  doc.text(`Total Received :${Number(config.totalReceived).toFixed(2)}`, marginLeft, currentY, { align: "left" });
