...
export const multerConfig = (
  ...
  const storage = multer.diskStorage({
    destination: function (req, file, cb) {
      try {
        ...
        const error = new AppError("Invalid file destination", 500);
        cb(error, false);
      }
    },
    filename: function (req, file, cb) {
      try {
        ...
        const error = new AppError("Failed to generate filename", 500);
        cb(error, false);
      }
    },
  });
  ...
  return multer({
    ...
    fileFilter: function (req, file, cb: (error: Error | null, acceptFile: boolean) => void) {
      try {
        if (filter.CONTENT.includes(file.mimetype)) {
          cb(null, true);
        } else {
          // Create a standard Error object instead of AppError
          cb(new Error(filter.MESSAGE), false);
        }
      } catch (err) {
        // Create a standard Error object instead of AppError
